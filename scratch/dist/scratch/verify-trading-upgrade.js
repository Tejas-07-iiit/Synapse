...
async function runVerification() {
    ...
    try {
        // 0. Ensure user exists and clean up database
        ...
        // Make sure we clear memory cache of PaperTradingEngine
        ...
        const pos1 = await PaperTradingEngine.openPosition(userId, "BTCUSDT", "LONG", entryPrice, null, // Auto-size
        stopLoss, takeProfit, 1, // 1x leverage
        10000.0, // Balance
        scalpingSettings, signalContext);
        if (!pos1) {
        ...
        if (pos1.stopLoss !== 992)
            throw new Error(`Incorrect stop loss calculated! Expected 992, got ${pos1.stopLoss}`);
        if (pos1.takeProfit !== 1012)
            throw new Error(`Incorrect take profit calculated! Expected 1012, got ${pos1.takeProfit}`);
        ...
        const posLowConf = await PaperTradingEngine.openPosition(userId, "ETHUSDT", "LONG", 2000, null, // Auto-size
        1900, 2200, 1, 10000.0, trendingSettings, { strategyId: "rsi-reversal", strategyName: "RSI Reversal", confidence: 50, marketContext: { regime: "RANGING" } });
        ...
        if (Math.abs(pctLowConf - 9.17) > 0.1)
            throw new Error("Incorrect low confidence sizing!");
        // Case 3b: Confidence = 70 (60-80). Expected sizing: 10% + (10/20)*20% = 20% of wallet
        console.log("- Case 3b: Medium confidence (60-80) sizing...");
        const posMedConf = await PaperTradingEngine.openPosition(userId, "SOLUSDT", "LONG", 100, null, 95, 110, 1, 10000.0, trendingSettings, { strategyId: "macd-momentum", strategyName: "MACD Momentum", confidence: 70, marketContext: { regime: "RANGING" } });
        const sizeMedConf = posMedConf ? posMedConf.entryPrice * posMedConf.quantity : 0;
        const pctMedConf = (sizeMedConf / 10000.0) * 100;
        console.log(`  [Confidence = 70]: Size = $${sizeMedConf.toFixed(2)} (${pctMedConf.toFixed(2)}%) (Expected: ~20.00%)`);
        if (Math.abs(pctMedConf - 20) > 0.1)
            throw new Error("Incorrect medium confidence sizing!");
        ...
        const posHighConfSafe = await PaperTradingEngine.openPosition(userId, "XRPUSDT", "LONG", 1.0, null, 0.9, 1.2, 1, 10000.0, trendingSettings, { strategyId: "momentum", strategyName: "Momentum", confidence: 95, marketContext: { regime: "TRENDING" } });
        ...
        console.log(`  [Confidence = 95, Trending]: Size = $${sizeHighConfSafe.toFixed(2)} (${pctHighConfSafe.toFixed(2)}%) (Expected: ~45.00%)`);
        if (Math.abs(pctHighConfSafe - 45) > 0.1)
            throw new Error("Incorrect high confidence safety rules met sizing!");
        ...
        const posHighConfUnsafe = await PaperTradingEngine.openPosition(userId, "ADAUSDT", "LONG", 0.5, null, 0.45, 0.6, 1, 10000.0, trendingSettings, { strategyId: "defensive", strategyName: "Defensive", confidence: 95, marketContext: { regime: "RANGING" } });
        ...
        if (Math.abs(pctHighConfUnsafe - 30) > 0.1)
            throw new Error("Incorrect high confidence safety rules NOT met sizing (should clamp to 30%)!");
        ...
        const btcPos = await PaperTradingEngine.openPosition(userId, "BTCUSDT", "LONG", 60000, 500, 59000, 62000, 1, 10000.0, trendingSettings, { strategyId: "macd-momentum", strategyName: "MACD Momentum", confidence: 80, marketContext: { regime: "TRENDING" } });
        if (!btcPos)
            throw new Error("Failed to place initial BTC position");
        // Case 4a: One-trade-per-symbol (BTCUSDT already open)
        console.log("- Case 4a: Attempting second trade on same symbol (BTCUSDT)...");
        const btcPos2 = await PaperTradingEngine.openPosition(userId, "BTCUSDT", "SHORT", 60000, 500, 61000, 58000, 1, 10000.0, trendingSettings, { strategyId: "ema-crossover", strategyName: "EMA Crossover", confidence: 80, marketContext: { regime: "TRENDING" } });
        console.log(`  [One-Trade-Per-Symbol]: result=${btcPos2 ? "OPENED" : "BLOCKED"} (Expected: BLOCKED)`);
        if (btcPos2 !== null)
            throw new Error("One-trade-per-symbol check failed!");
        ...
        const ethPos = await PaperTradingEngine.openPosition(userId, "ETHUSDT", "LONG", 3000, 500, 29000, 32000, 1, 10000.0, trendingSettings, { strategyId: "macd-momentum", strategyName: "MACD Momentum", confidence: 80, marketContext: { regime: "TRENDING" } });
        console.log(`  [One-Trade-Per-Strategy]: result=${ethPos ? "OPENED" : "BLOCKED"} (Expected: BLOCKED)`);
        if (ethPos !== null)
            throw new Error("One-trade-per-strategy check failed!");
        ...
        const ethPosCorrelated = await PaperTradingEngine.openPosition(userId, "ETHUSDT", "LONG", 3000, 500, 29000, 32000, 1, 10000.0, trendingSettings, { strategyId: "ema-crossover", strategyName: "EMA Crossover", confidence: 80, marketContext: { regime: "TRENDING" } });
        console.log(`  [Correlation Filter]: result=${ethPosCorrelated ? "OPENED" : "BLOCKED"} (Expected: BLOCKED)`);
        if (ethPosCorrelated !== null)
            throw new Error("Correlation risk filter failed!");
        ...
        const ethPosAllowed = await PaperTradingEngine.openPosition(userId, "ETHUSDT", "SHORT", 3000, 500, 31000, 2800, 1, 10000.0, trendingSettings, { strategyId: "ema-crossover", strategyName: "EMA Crossover", confidence: 80, marketContext: { regime: "TRENDING" } });
        console.log(`  [Non-Correlated Trade]: result=${ethPosAllowed ? "OPENED" : "BLOCKED"} (Expected: OPENED)`);
        if (ethPosAllowed === null)
            throw new Error("Legitimate opposite direction trade blocked by correlation filter!");
