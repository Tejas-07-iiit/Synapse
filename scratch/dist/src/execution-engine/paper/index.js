...
export class PaperTradingEngine {
    ...
    static registerDbHandler(handler) {
        this.dbHandler = handler;
    }
    static positions = new Map(); // positionId -> Position
    static lastDbUpdate = new Map(); // positionId -> timestamp
    static executionLocks = new Set(); // Mutex for concurrent symbol processing
    static symbolCooldowns = new Map(); // symbol -> cooldown expiry timestamp
    /**
     * Initializes virtual positions by loading any open ones from the database on startup.
     */
    static async loadActivePositions(userId) {
        try {
            let positionsList = [];
            if (this.dbHandler) {
     ...
     */
    static async updatePrices(symbol, currentPrice, currentHigh, currentLow) {
        const sym = symbol.toUpperCase();
        const activePositions = Array.from(this.positions.values()).filter((p) => p.symbol === sym && p.status === "OPEN");
     ...
     */
    static async openPosition(userId, symbol, direction, price, sizeUsdt = null, stopLoss = null, takeProfit = null, leverage = 1, explicitBalance, explicitSettings, signalContext) {
        const sym = symbol.toUpperCase();
        ...
        try {
            ...
            let alreadyOpen = Array.from(this.positions.values()).some((p) => p.symbol === sym && p.status === "OPEN" && p.userId === userId);
            if (alreadyOpen) {
                return null;
            }
            ...
            try {
                ...
                if (this.dbHandler) {
                    ...
                    dbOpen = dbPositions.some((p) => p.symbol === sym && p.status === "OPEN");
                }
                else if (typeof window !== "undefined") {
                    const res = await fetch(`/api/positions?userId=${encodeURIComponent(userId)}`);
                    const body = await res.json();
                    if (body.success && Array.isArray(body.positions)) {
                        dbOpen = body.positions.some((p) => p.symbol === sym && p.status === "OPEN");
                    }
                }
                if (dbOpen) {
                    alreadyOpen = true;
            ...
            const alreadyOpenForStrategy = Array.from(this.positions.values()).some((p) => p.userId === userId && p.status === "OPEN" && p.strategyId === strategyId && strategyId !== "manual");
            const alreadyOpenInSameDirection = Array.from(this.positions.values()).some((p) => p.userId === userId && p.status === "OPEN" && p.direction === direction && p.symbol !== sym);
            ...
            try {
                if (this.dbHandler) {
                    ...
                    const openPositions = Array.from(this.positions.values()).filter((p) => p.userId === userId && p.status === "OPEN");
            ...
            if (sizeUsdt === null) {
                if (signalContext && signalContext.confidence !== undefined) {
                    const confidence = signalContext.confidence;
                    ...
                    if (confidence < 60) {
                        ...
                        pct = 10 + ((confidence - 60) / 20) * 20;
                    }
                    else {
                        pct = 30 + ((confidence - 80) / 20) * 20;
                    }
                    // Safety Rules
                    if (pct > 30) {
                        const isSafetyMet = confidence > 90 &&
            ...
            const riskResult = RiskEngine.validateOrder(dummyOrder, userActivePositionsCount, alreadyOpen, leverage, availableBalance, explicitSettings, alreadyOpenForStrategy, alreadyOpenInSameDirection);
     ...
     */
    static async closePosition(positionId, exitPrice, reason = "MANUAL") {
